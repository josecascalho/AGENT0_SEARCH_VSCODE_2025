...
class Agent:
    ...
    def run(self):
        ...
        self.frontier_nodes.add(root)
        #nd = self.frontier_nodes.pop()
        nd = self.frontier_nodes.get()
        ...
        for action in ["south","west","north","east"]:
            ...
            if not self.in_visited_nodes(nd) and not self.state_has_obstacle(nd):
                ...
                self.frontier_nodes.add(nd)
                self.mark(nd.get_state(),"red")
        #self.frontier_nodes.print_stack("After first possible actions")
        self.frontier_nodes.print_queue("After first possible actions")
        ...
        while end == False:
            ...
            node_to_expand = self.frontier_nodes.get()
            ...
            for act in ["west","south","north","east"]:
